...
class LoggerDebug {
  ...
  String get time =>
      "${"${DateTime.now().hour}".padLeft(2, '0')}:${"${DateTime.now().minute}".padLeft(2, '0')}:${"${DateTime.now().second}".padLeft(2, '0')}:${"${DateTime.now().millisecond}".padLeft(2, '0')}"
          .padLeft(9, '0');
  ...
  black(String message, [String? title]) {
    return log(
      ...
      name:
          "${LogColors.blue}$time $headColor${title ?? constTitle ?? ""}${LogColors.reset}",
      level: 2000,
    );
  }
